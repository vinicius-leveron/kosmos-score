-- ============================================================================
-- ADD RAIO-X KOSMOS LEAD MAGNET TYPE
-- Adiciona o tipo raio-x-kosmos para o novo lead magnet Raio-X KOSMOS
-- ============================================================================
...
-- Adicionar nova constraint com todos os tipos (novos + legacy + raio-x)
ALTER TABLE public.lead_magnet_results
...
'Tipo do lead magnet:
  - maturity-diagnostic: Diagnóstico de Maturidade de Ecossistema
  - raio-x-kosmos: Raio-X KOSMOS - Diagnóstico rápido do ecossistema digital (NOVO)
  - stop-launching-simulator: Simulador "E Se Você Parar de Lançar"
  - ecosystem-blueprint: Blueprint de Ecossistema
...
-- ============================================================================
-- FUNÇÃO: Mapeia lead magnets para stages do pipeline
-- Atualiza a função de sync para suportar o novo raio-x-kosmos
-- ============================================================================
...
BEGIN
  ...
  IF v_pipeline_id IS NOT NULL THEN
    ...
    -- Para raio-x-kosmos, usar total_score com faixas 0-5, 6-11, 12+
    ELSIF NEW.lead_magnet_type = 'raio-x-kosmos' THEN
      ...
      WHERE pipeline_id = v_pipeline_id
        AND name = CASE
          WHEN COALESCE(NEW.total_score, 0) >= 12 THEN 'high_score'
          WHEN COALESCE(NEW.total_score, 0) >= 6 THEN 'medium_score'
          ELSE 'low_score'
        END;
