-- ============================================================================
-- Fix sync_lead_magnet_to_contact trigger - remove source from contact_orgs
-- The source column exists only on contacts table, not contact_orgs
-- ============================================================================
...
BEGIN
  ...
  -- Buscar ou criar contact_org (SEM coluna source - ela não existe nesta tabela)
  ...
  IF v_contact_org_id IS NULL THEN
    ...
    VALUES (
      ...
      'active',
      COALESCE(NEW.score_breakdown, '{}'::jsonb)
    )
