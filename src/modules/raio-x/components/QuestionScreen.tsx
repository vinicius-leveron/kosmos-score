...
import type { RaioXQuestion, RaioXAnswer, RaioXOption } from '../lib/types';

interface QuestionScreenProps {
  question: RaioXQuestion;
  currentIndex: number;
  totalQuestions: number;
  currentAnswer: RaioXAnswer | undefined;
  onAnswer: (answer: RaioXAnswer) => void;
  onPrevious: () => void;
  onNext: () => void;
  canGoBack: boolean;
...
export function QuestionScreen({
  ...
  const blockLabel = BLOCK_LABELS[question.block] ?? '';

  const selectedValue = currentAnswer?.value;
  const textValue = currentAnswer?.type === 'text' ? currentAnswer.value : '';

  const handleOptionClick = (option: RaioXOption) => {
    onAnswer({ type: 'single', value: option.value, numericValue: option.numericValue });
  };

  const handleTextChange = (value: string) => {
    onAnswer({ type: 'text', value });
  };

  const canProceed = question.required
    ? currentAnswer != null && currentAnswer.value.trim() !== ''
    : true;
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-2">
            <span className="text-kosmos-gray text-sm">
              {blockLabel} — Pergunta {currentIndex + 1} de {totalQuestions}
            </span>
        ...
        <div className="card-structural p-6 md:p-8">
          ...
          <div className="mb-8">
            ...
            {question.subtitle && (
              <div className="flex items-start gap-2 mt-3">
                ...
                <p className="text-kosmos-gray text-sm">{question.subtitle}</p>
              </div>
            )}
          </div>
          ...
          {question.type === 'text' && (
            <textarea
              ...
              rows={4}
              className="w-full rounded-xl border-2 border-border bg-kosmos-black/50 p-4 text-kosmos-white placeholder:text-kosmos-gray/50 focus:border-kosmos-orange focus:ring-1 focus:ring-kosmos-orange/20 focus:outline-none resize-none transition-colors"
